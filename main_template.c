...
void user_prompt_loop()
{
   ...
   // When true, the Shell will stop prompting for more user information
    ...
    //free(tokens);
	    ...
	    for(int i = 0; tokens[i] != NULL; ++i){
	    	char *coolString = unescape(tokens[i], NULL);
	    	//printf("Unescape: %s", coolString);
	    	tokens[i] = coolString;
	    }
    ...
    */
    ...
    for(int i = 0; userArguments[i] != NULL; i++){
        ...
        if(userArguments[i][0] == '~'){
            ...
            for(int j = 1; j < strlen(userArguments[i]); j++){
...
int write_to_history(char *userInput){
    ...
    if(shiftUp == true){
        //printf("SHIFTING UP\n");
        for(int i = 0; i < 9; i++){
            fprintf(theFile, "%s", historyList[i]);
        }
